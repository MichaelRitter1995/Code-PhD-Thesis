...
def main():
    ...
    transcripts_df = pd.read_csv("X0.0-24000.0_Y0.0-24000.0_filtered_transcripts.csv",
    ...
    cells = np.unique(transcripts_df["cell_id"])[1:]
    ...
    for index, row in transcripts_df.iterrows():
        ...
        cell = row['cell_id']
        overlaps_nucleus = row['overlaps_nucleus']
        nucleus_distance = row['nucleus_distance']

	# Ignore transcript below user-specified cutoff
        if overlaps_nucleus==0 or nucleus_distance > args.distance:
            continue
